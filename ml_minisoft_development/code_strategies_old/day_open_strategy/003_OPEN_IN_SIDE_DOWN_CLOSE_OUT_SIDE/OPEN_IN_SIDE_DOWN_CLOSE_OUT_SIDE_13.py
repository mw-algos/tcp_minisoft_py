...
for row_number_, row_record_ in df_bank_nifty_days_data.iloc[1:].iterrows():
    ...
    for row_number, row_record in df_bn_st_7_3.iloc[1:].iterrows():
        ...
        stop_loss = ((current_candle_dict_info['cur_close'] - current_candle_dict_info['pre_low']) / current_candle_dict_info['cur_open'])
        diff_super_trend = abs(round(stop_loss * 100, 2))

        primary_entry_condition_sell = (profit_loss_day_df.shape[0] == 0) and (current_candle_dict_info['candle_type'] == 'red candle') and row_record.low < current_candle_dict_info['cur_low']

        if row_number == 1:
            second_entry_condition_buy = (current_candle_dict_info['candle_type'] == 'red candle') and row_record.low < current_candle_dict_info['cur_low']
            second_entry_condition_sell = row_record.low > current_candle_dict_info['cur_low']


        if primary_entry_condition_sell and second_entry_condition_buy and row_record.close < current_candle_dict_info['cur_low']:
        ...
        elif primary_entry_condition_sell and current_record == 'up' and profit_loss_day_df_new.shape[0] > 2 and row_record.close > second :
            profit_loss_df = {'date_on_str': row_record.date_on_str,
                              'date': current_candle_dict_info['date'],
                              'instrument': current_candle_dict_info['candle_width'],
                              'traded_date_time': row_record.date,
        ...
        elif primary_entry_condition_sell and current_record == 'down' and profit_loss_day_df_new.shape[0] > 2 and  row_record.close < second :
            profit_loss_df = {'date_on_str': row_record.date_on_str,
                              'date': current_candle_dict_info['date'],
                              'instrument': current_candle_dict_info['candle_width'],
                              'traded_date_time': row_record.date,
