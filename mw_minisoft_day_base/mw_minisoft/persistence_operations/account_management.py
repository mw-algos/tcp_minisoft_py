...
def calculate_expiry_date():
    ...
    for ticks_info_position, ticks_info_record in trade_inst.iterrows():
        ...
        instruments = instruments[(instruments['Scrip code'] == ticks_info_record.inst_name) & (instruments['Option type'] == 'CE')]
        instruments[['script', 'year', 'mon', 'day', 'strike_price']] = instruments['Symbol Details'].str.split(' ', expand=True).iloc[:, 0:5]
        instrument_days = (pd.to_datetime((instruments['year'] + instruments['mon'] + instruments['day']), format='%y%b%d')).dt.date
        ...
        expiry_day = ((pd.to_datetime((expiry_day_record['year'] + expiry_day_record['mon'] + expiry_day_record['day']), format='%y%b%d')).date())
...
def update_instr_file():
    ...
    for ticks_info_position, ticks_info_record in trade_inst.iterrows():
            ...
            instruments[['script', 'year', 'mon', 'day', 'strike_price']] = instruments['Symbol Details'].str.split(' ', expand=True).iloc[:, 0:5]
            instruments['expiry_date'] = (pd.to_datetime((instruments['year'] + instruments['mon'] + instruments['day']), format='%y%b%d')).dt.date
            instruments = instruments[instruments['expiry_date'] == pd.to_datetime(ticks_info_record.inst_expiry_date)]
            instruments_buy = instruments[(instruments['Option type'] == 'PE') & (pd.to_numeric(instruments['strike_price']) > float(ticks_info_record.inst_day_high))].sort_values(by=['strike_price'], ascending=True).head(1)
            instruments_sell = instruments[(instruments['Option type'] == 'CE') & (pd.to_numeric(instruments['strike_price']) < float(ticks_info_record.inst_day_low))].sort_values(by=['strike_price'], ascending=False).head(1)
...
def update_instrument_file(ticks_indi_create, ticks_info_position, ticks_info_record):
    ...
    trade_inst = trade_inst[trade_inst['instrument_name'] == ticks_info_record.inst_segment +":"+ ticks_info_record.inst_name]
